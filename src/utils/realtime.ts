...
async function createDocument<T extends FirestoreDocument>(
  ...
  try {
    ...
    logger.error(
      `Error creating document in path ${path}: ${
...
async function createDocumentWithId<T extends FirestoreDocument>(
  ...
  try {
    ...
    logger.error(
      `Error creating document with ID: ${docId} in path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function getDocumentById<T extends FirestoreDocument>(
  ...
  try {
    ...
    logger.error(
      `Error fetching document with ID: ${docId} from path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function getAllDocuments<T extends FirestoreDocument>(
  ...
  try {
    ...
    logger.error(
      `Error fetching all documents from path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function queryDocuments<T extends FirestoreDocument>(
  ...
  try {
    ...
    if (options.startAfter !== undefined) {
      const startValue =
        typeof options.startAfter === 'object'
          ? (options.startAfter as any).id
          : options.startAfter
    ...
    if (options.endBefore !== undefined) {
      const endValue =
        typeof options.endBefore === 'object'
          ? (options.endBefore as any).id
          : options.endBefore
    ...
    logger.info(
      `Query returned ${documents.length} documents from path: ${path}`,
    )
    return {
      success: true,
      data: documents,
    ...
    logger.error(
      `Error querying documents from path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function updateDocument<T extends FirestoreDocument>(
  ...
  try {
    ...
    logger.error(
      `Error updating document with ID: ${docId} in path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function setDocument<T extends FirestoreDocument>(
  ...
  try {
    ...
    logger.error(
      `Error setting document with ID: ${docId} in path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function deleteDocument(
  ...
  try {
    ...
    logger.error(
      `Error deleting document with ID: ${docId} from path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function deleteDocumentsByQuery(
  ...
  try {
    ...
    logger.error(
      `Error deleting documents by query from path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
  ...
  try {
    ...
    for (const { type, path, docId, data } of operations) {
      if (type === 'create') {
        ...
        const currentData =
          (await realtimeDb.ref(`${path}/${docId}`).once('value')).val() || {}
        updates[`${path}/${docId}`] = {
          ...currentData,
          ...data,
          updatedAt: now,
...
async function runDbTransaction<T>(
  ...
  try {
    ...
    logger.error(
      `Error executing database transaction on path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
...
async function documentExists(path: string, docId: string): Promise<boolean> {
  try {
    logger.info(
      `Checking if document with ID: ${docId} exists in path: ${path}`,
    )
    const snapshot = await realtimeDb.ref(`${path}/${docId}`).once('value')
    const exists = snapshot.exists()
    logger.info(`Document with ID: ${docId} in path ${path} exists: ${exists}`)
    return exists
  } catch (error) {
    logger.error(
      `Error checking if document with ID: ${docId} exists in path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return false
...
async function countDocuments(
  ...
  try {
    ...
    logger.error(
      `Error counting documents in path ${path}: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`,
    )
    return {
